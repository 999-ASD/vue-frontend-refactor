...
export const useHospitalStore = defineStore('hospital', () => {
  ...
  const filteredDoctors = computed(() => (dept) => {
    if (!dept) return doctors.value
    return doctors.value.filter(d => d.department === dept)
  })

  function setCurrentPatient(patient) { currentPatient.value = patient }
  ...
  function createMedicalRecord(patient) {
    return {
      ...patient,
